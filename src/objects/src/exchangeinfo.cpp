...
ExchangeInfo::ExchangeInfo(std::string_view exchangeNameStr, std::string_view makerStr, std::string_view takerStr,
                           ...
                           const APIUpdateFrequencies &apiUpdateFrequencies, int minPublicQueryDelayMs,
  ...
  if (log::get_level() <= log::level::trace) {
    ...
    log::trace(" - General update frequencies   : {} ms for public, {} ms for private", minPublicQueryDelayMs,
               minPrivateQueryDelayMs);
