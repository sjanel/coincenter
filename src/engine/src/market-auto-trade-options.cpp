...
namespace {
auto GetFieldOrThrow(const json::container &data, std::string_view fieldName) {
...
MarketAutoTradeOptions::MarketAutoTradeOptions(const json::container &data)
    : _accounts(),
  ...
  std::ranges::transform(*accountsIt, std::back_inserter(_accounts), [](const json::container &accountJson) {
    return string(accountJson.get<std::string_view>());
  });
  ...
  std::ranges::transform(*stopCritJsonIt, std::back_inserter(_stopCriteria), [](const json::container &stopCrit) {
