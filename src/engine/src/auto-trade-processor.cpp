...
AutoTradeProcessor::AutoTradeProcessor(const AutoTradeOptions &autoTradeOptions)
  ...
  for (const auto &[publicExchangeName, publicExchangeAutoTradeOptions] : autoTradeOptions) {
    ...
    for (const auto &[market, marketAutoTradeOptions] : publicExchangeAutoTradeOptions) {
      ...
      for (std::string_view account : marketAutoTradeOptions.accounts()) {
        marketStatus.privateExchangeNames.emplace_back(publicExchangeName.name(), account);
      }
    }
    ++publicExchangePos;
  }
...
namespace {
const MarketAutoTradeOptions &GetMarketAutoTradeOptions(
    Market market, const PublicExchangeAutoTradeOptions &publicExchangeAutoTradeOptions) {
...
bool IsQueryTooEarly(TimePoint nowTs, const auto &marketStatus,
                     const PublicExchangeAutoTradeOptions &publicExchangeAutoTradeOptions) {
  const auto &marketAutoTradeOptions = GetMarketAutoTradeOptions(marketStatus.market, publicExchangeAutoTradeOptions);
  return marketStatus.lastQueryTime + marketAutoTradeOptions.repeatTime() > nowTs;
}
}  // namespace
  ...
  for (ExchangeStatus &exchangeStatus : _exchangeStatusVector) {
    ...
    if (IsQueryTooEarly(ts, selectedMarketStatus, publicExchangeAutoTradeOptions)) {
      const auto repeatTime =
          GetMarketAutoTradeOptions(selectedMarketStatus.market, publicExchangeAutoTradeOptions).repeatTime();
