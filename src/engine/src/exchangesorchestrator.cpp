...
MarketsPerExchange ExchangesOrchestrator::pullAvailableMarketsForReplay(TimeWindow timeWindow,
  ...
  MarketsPerExchange marketsPerExchange(selectedExchanges.size());
  _threadPool.parallelTransform(
      selectedExchanges.begin(), selectedExchanges.end(), marketsPerExchange.begin(), [timeWindow](Exchange *exchange) {
        return std::make_pair(exchange, exchange->apiPublic().pullAvailableMarketsForReplay(timeWindow));
      });
  return marketsPerExchange;
}
