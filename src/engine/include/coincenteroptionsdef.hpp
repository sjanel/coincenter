...
struct CoincenterAllowedOptions : private CoincenterCmdLineOptionsDefinitions {
  ...
  static constexpr CommandLineOptionWithValue value[] = {
      ...
      {{{"Order", 4030}, "--sim", "", kSimulationMode}, &OptValueType::isSimulation},
      ...
      {{{"Withdraw and deposit", 6010},
        "--sim",
        "",
        "Simulation mode: check if withdraw can be done (by retrieving deposit info of destination and source "
        "exchange) instead of actually performing the withdraw."},
       &OptValueType::isSimulation},
      {{{"Withdraw and deposit", 6020},
        "withdraw-apply-all",
        "<cur,from-to>",
        "Withdraw all available amount instead of a specified amount.\nBenefits from same options as withdraw-apply."},
