...
TradeRangeResultsStats ValidateRange(VectorType &vec, TimePoint earliestPossibleTime) {
  ...
  auto nbInvalidObjects = erase_if(vec, [](const auto &obj) { return !obj.isValid(); });
  if (nbInvalidObjects != 0) {
    log::error("{} {}(s) with invalid data detected", nbInvalidObjects, kObjName);
  }

  auto nbUnsortedObjectsRemoved = erase_if(vec, [&earliestPossibleTime](const auto &obj) {
...
TradeRangeStats MarketTraderEngine::tradeRange(MarketOrderBookVector &&marketOrderBooks,
  ...
  auto tradesBeg = publicTrades.begin();
  for (const MarketOrderBook &marketOrderBook : marketOrderBooks) {
    ...
    const auto tradesEnd = std::partition_point(
        tradesBeg, publicTrades.end(),
        [marketOrderBookTs](const auto &publicTrade) { return publicTrade.time() < marketOrderBookTs; });

    const std::span<const PublicTrade> publicTradesForThisOrderbook(tradesBeg, tradesEnd);

    // Call the user algorithm trading engine and retrieve its decision for next move
    const TraderCommand traderCommand = _marketTrader->trade(marketOrderBook, publicTradesForThisOrderbook);
